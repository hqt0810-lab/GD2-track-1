...
| Test | Cheap model (Flash-Lite) | Strong model (Sonnet 4.6) | Chênh lệch rõ rệt? |
|---|---|---|---|
| Test 1 (Điểm đến) | ✓ Đầy đủ 3+ địa điểm, phân bổ ngày hợp lý từ RAG | ✓ Chi tiết hơn, thêm tips ẩm thực/văn hoá địa phương | □ Có · ☑ Không |
| Test 2 (Visa context) | △ Trả lời đúng nhưng bỏ qua signal "rules changed" — không trigger web search | ✓ Nhận diện "rules changed recently" → gợi ý tra cứu thêm, cảnh báo policy có thể thay đổi | ☑ Có · □ Không |
| Test 3 (Multi-intent) | △ Chỉ trả lời phần thời tiết, bỏ qua yêu cầu booking cruise | ✓ Nhận ra 2 intent — trả lời weather + đề xuất chuyển sales cho booking | ☑ Có · □ Không |
| Test 4 (Khiếu nại) | △ Cố xin lỗi và đề nghị giải pháp thay vì chuyển manager ngay | ✓ Nhận diện khiếu nại nghiêm trọng → chuyển ngay manager, không tự xử lý | ☑ Có · □ Không |
...
```text
Flash-Lite đủ tốt cho Test 1 (Guide) vì RAG đã cung cấp đủ context — chênh lệch chất lượng không đáng kể.
Nhưng Test 2 (Visa) Flash-Lite không nhận ra signal "rules changed recently" cần web search, và Test 3 (Multi-intent) chỉ xử lý 1/2 intent.
Test 4 (Khiếu nại) là rủi ro lớn nhất: Flash-Lite cố tự giải quyết thay vì escalate → nguy cơ pháp lý.
Sonnet 4.6 bắt hết nuance ở cả 4 tests nhưng cost gấp 30×.
→ Quyết định cuối: Smart Navigator Mix — Flash-Lite cho Guide (đủ tốt), Sonnet 4.6 cho Visa + Weather + Khiếu nại (cần nuance). LLM classifier bắt buộc để routing chính xác.
```
